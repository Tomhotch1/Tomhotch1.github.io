...
class Character:
    ...
    def __init__(self, map, name='', location='', human=False):
        """
        ...
        @param human, True if the character is the player, false otherwise.
        """
        self.map = map
        self.inventory = []
        self.name = name
    ...
    def __str__(self):
        """
        Converts the Character object to a string for printing.
        """
        return 'Name: {}\nLocation: {}\nHuman: {}\nMovement speed: {}\nInventory: {}\n'.format(
            self.name, self.location, self.human, self.inventory
        )
    ...
    def moveTo(self, new_room_name):
        ...
        """
        # if the moving character is the robot, no map updates are needed
        if not self.human:
            ...
            print('Moved to {}.'.format(self.location.name))
