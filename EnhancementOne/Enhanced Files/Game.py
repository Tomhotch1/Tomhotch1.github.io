...
class Game:
    ...
    def init_characters(self):
        ...
        """
        self.player = Character.Character(self.map, 'Tom', 'Planetary Elevator', True)      # player starting position is Planetary Elevator
        self.enemy = Character.Character(self.map, 'Pap-AI', 'Escape Pods 2', False)        # robot starting position is Escape Pods 2
    ...
    def process_command(self, command, direction):
        ...
        @return win, the game's status, True if game is over and False otherwise.
        """
        if command == 'move':
            # Attempt to move in the given direction
            self.player.attempt_move(direction) 
