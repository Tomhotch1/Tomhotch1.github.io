...
class Map:
    ...
    def __init__(self, room_list_path, grid_size):
        ...
        self.generator = MapGenerator.MapGenerator(room_list_path, self.grid_size, 'dynamic')
        self.room_grid, self.teleporters = self.generator.generate_map()
        self.test_print_map()     # DEBUG: Prints map with all room labels
    ...
    def get_non_empty(self, room):
        """
        Gets list of all adjacent rooms that are not empty.
        
        @param room, the room from which to get non-empty rooms.
        @return non_empty, a list of non-empty adjacent rooms.
        """
        # start by getting all adjacent rooms
        adjacent_rooms = self.get_accessible(room)
        non_empty = []

        # Add all non-empty adjacent rooms to non_empty
        for curr_room in adjacent_rooms:
            if curr_room.type != 'empty':
                non_empty.append(curr_room)
    ...
    def next_teleporter(self, curr_room):
        ...
        @param curr_room, the current room location, must contain a teleporter.
    ...
    def test_print_map(self):
        ...
        for row in self.room_grid:
            for room in row:
                ...
                ax.text(
                    room.x_pos + 0.5, room.y_pos + 0.5, display_text,
                    ha='center', va='center', fontsize=6, color='yellow'
                )
